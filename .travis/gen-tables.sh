...
function gen_functions()
{
	...
	for L in ${LINES[*]}; do
		if [[ "$L" =~ ^[[:space:]*]*((url|address|bool|command|dump|long|magic|mbtable|number|path|quad|regex|cs|sort|string|crypto*_|driver_|getdns|imap_|log_|mbox_|mmdf_|mutt_|mx_|nm_|nntp_|pgp_|pop_|raw_|rfc1524_|rfc2047_|rfc2231_|serial_|smime_|tunnel_).*)[[:space:]]-[[:space:]](.*) ]]; then
			FUNC="${BASH_REMATCH[1]}"
			DESC="${BASH_REMATCH[3]}"
			echo " * | $FUNC() | $DESC |"
